...
class EMFINet(nn.Module):
    ...
    def forward(self, x):
        ...
        # -------------Return--------------#
        return torch.sigmoid(edge_out), torch.sigmoid(label1_out), torch.sigmoid(label2_out), torch.sigmoid(
            label3_out), torch.sigmoid(labelbdg_out), \
               torch.sigmoid(label4_out), torch.sigmoid(label5_out), torch.sigmoid(label6_out), torch.sigmoid(
            label7_out), torch.sigmoid(label8_out)
